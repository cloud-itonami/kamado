(ns kamado.methods.social
  "social.cljc — kamado DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from kamado's own
...
(defn draft-observation-post
  "A dry-run observation post over kamado's own domain (Closed-loop carbon REFINING + fossil-refinery DECOMMISSIONING/TRANSITION + refinery OBSERVATION (竈 = the sacred hearth/furnace; 竈神/荒神 the…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "kamado R0: live social posting is Council Lv6+ + operator + "
